...
```
or in LuCI: **Status → Overview** (firmware version). `23.*`/`24.*` → take the
`.ipk`; `25.*` → take the `.apk`.
...
- `htdocs/luci-static/uniwrt/js/uniwrt.js` — relocates LuCI's menu into the left
  rail, adds the collapse + light/dark controls, and styles the login page. It
  watches for LuCI's async-rendered menu and **degrades gracefully**: if it
  can't find the menu, the styling and dark mode still apply and the native nav
  is left in place.
- `luasrc/view/themes/uniwrt/*.htm` (Lua, for 23.05) and
...
- The CSS and JS are the dependable core. The header/footer templates are the
  ...
  Lua (23.05) and ucode (24.10/25.12) engines.
