...
apk add --allow-untrusted ./luci-theme-uniwrt-2.0.14-r1.apk
```
...
opkg install ./luci-theme-uniwrt_2.0.14-1_all.ipk
```
...
This repo ships `.github/workflows/build.yml`, which builds all three branches with the official `openwrt/gh-action-sdk` and uploads both `.ipk` and `.apk` artifacts. Push a `v*` tag to also attach them to a GitHub Release:
...
```
uniwrt-luci/
├── .github/workflows/build.yml          # dual IPK + APK CI matrix
└── luci-theme-uniwrt/
    ├── Makefile
